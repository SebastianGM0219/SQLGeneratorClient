...
function App() {
  ...
  const crosstabSelectors = useSelector(
    (state) => state.query.crosstabSelectorFields
  );
  const crosstabColumns = useSelector(
    (state) => state.query.crosstabColumnFields
  );
  const crosstabValues = useSelector(
    (state) => state.query.crosstabValueFields
  );
  ...
  const handleRun = () => {
    ...
    selectFields.map((item, index) => {
      const {
        data: { table, field, header_name },
      } = item;
      selectQuery += `${table}.${field} as ${header_name}`;
      if (index !== selectFields.length - 1) selectQuery += ", \n       ";
    });
    ...
    joinFields.map((item) => {
      ...
      let joinString = `${joinTypeArray[joinType]} ${joinTable} ON`;
      for (let i = 0; i < LFields.length; i++) {
        joinString += ` ${LFields[i]}${OperatorTypeArray[Operators[i]]}${
          RFields[i]
        }`;
        if (i !== LFields.length - 1) {
          joinString += " AND";
    ...
    filterFields.map((item, index) => {
      ...
      if (item.typeOperator === "Like" || item.typeOperator === "Not Like") {
        condition =
          ...
          item.field +
          " " +
          item.typeOperator.toUpperCase() +
          " " +
          `'%${item.value}%'`;
      } else
        condition =
          item.table + "." + item.field + item.typeOperator + valueString;
      whereQuery += condition;
    });
    ...
    if (isCrossTab && crosstabColumns.length > 0) {
      crosstabColumns.forEach((item, index) => {
        ...
        if (index < crosstabColumns.length - 1) groupByQuery += string;
        if (index < crosstabColumns.length - 2) {
          groupByQuery += ",";
        }
      });
      ...
      crosstabColumns.map((item, index) => {
        if (item.id !== "none") {
          ...
          cSelectQuery += `${table}.${field}`;
          if (index !== selectFields.length - 1) cSelectQuery += ",\n       ";
        }
      });
      crosstabValues.map((item) => {
        const {
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <AppBar>
        ...
        >
          ...
          >
            {" "}
            <SaveIcon />{" "}
            <Typography style={{ fontSize: 12 }}>Connect</Typography>
          </Button>
          <Button
            variant="inherit"
          ...
          >
            {" "}
            <WifiIcon />{" "}
            <Typography style={{ fontSize: 12 }}>Run query</Typography>
          </Button>
          <Button
            variant="inherit"
            onClick={handleReset}
          ...
          >
            {" "}
            <RefreshIcon />{" "}
            <Typography style={{ fontSize: 12 }}>Reset</Typography>
          </Button>
          <Button
            variant="inherit"
          ...
          >
            {" "}
            <DescriptionIcon />{" "}
            <Typography style={{ fontSize: 12 }}>Create View</Typography>
          </Button>
      ...
      <Modal
        open={openModal}
        param={{ title: "Failed", content: "An error occured!" }}
        handleClose={handleCloseModal}
      />
      <AddTableDialog
        open={addDialog}
        handleAddTableClose={handleAddTableClose}
        handleTableClose={handleTableClose}
      />
      <ParameterDialog
        open={paramDialog}
        handleParamClose={handleParamClose}
        handleRun={handleRun}
      />
      <CreateViewDialog
        open={createViewDialog}
        handleCreateViewClose={handleCreateViewClose}
        SaveView={SaveView}
      />
