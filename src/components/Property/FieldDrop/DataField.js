...
import { removeFieldCalcDrop } from "../../../slices/query";
...
export const DataField = (props) => {
  ...
  const handleClick = (e) => {
    dispatch(removeFieldCalcDrop({id}));
  }

  const handleSelect= (e) => {
    handleParent(props.index);
//   props.exchangeByNumber(index);
  ...
  return (
    <Box>
      ...
      {
        id !== 'none' &&
        <Box className={classes.itemStyle} onMouseOver={showCloseButton} onMouseOut = {hideCloseButton} onClick={handleSelect}>
